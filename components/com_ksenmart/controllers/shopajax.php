...
class KsenMartControllerShopAjax extends JController{
            ...
            if($_REQUEST['state'] == $_SESSION['state']) {
				...
				if(window.opener != null && !window.opener.closed) {
					if (window.opener.getElementById('subcribe').checked==true)
						window.opener.location='<?php echo JRoute::_('index.php?option=com_ksenmart&view=shopprofile&Itemid='.KMSystem::getShopItemid())?>';
					else
	...
	function get_order_shippings()
	{
		...
		if (count($shippings)>0)
		{
			...
			<div class="shipping">
				...
				<span class="grey-span"><?php echo JText::_($ship->title)?><?php echo ($cost!=0?' — '.KMPrice::showPriceWithoutTransform($cost):'')?></span>
				<span class="shipping-descr">(доставка не позднее <?php echo KMShipping::getShippingDate($ship->id)?>)</span>
			</div>			
			<?
			}
		}
	...
	function callback()
	{
		...
		if ($return)	
			echo 'В ближайшее время мы с вами свяжемся .';
		else
			echo 'Ошибка отправки запроса . Попробуйте позже .';
		exit();	
	}
	...
	function question()
	{
		...
		if ($return)	
			...
			echo 'Ошибка отправки вопроса . Попробуйте позже .';
		exit();	
	}	
	...
	function get_product_price_with_properties()
	{
		...
		$properties   = KMProducts::getProperties($pid, $prop_id, $val_prop_id);
		$price        = KMProducts::getProductPrices($pid)->price;
	...
	function validate_in_stock()
	{
		...
		$product=KMProducts::getProduct($id);
	...
	function save_variable()
	{
		...
		if ($name!='')
			$app->setUserState('com_ksenmart.'.$name,$value);
		$app->close();
	}
	...
	function get_transform_price()
	{
		...
		echo KMPrice::showPriceWithTransform($price);	
	}
