...
class KsenMartModelShopOpenOrder extends JModelKMAdmin {
    var $_order_id = null;
    ...
    public function getUser() {
        return KMUsers::getUser();
    }

    public function getProduct() {
        $id = JRequest::getVar('id', 0);
        $row = KMProducts::getProduct($id);
    ...
    public function createOrder($flag = 2) {
        ...
        if(!empty($prd_id)){
            $prd        = KMProducts::getProduct($prd_id);
        }
        ...
        if($prd->type == 'set') {
            $properties      = $this->getPropertiesVByIds(KMProducts::getSetRelatedIds($prd_id));
