 ...
 * @access public
 */
class KsenMartModelShopProfile extends JModelKMAdmin {
    ...
    /**
     * KsenMartModelShopProfile::__construct()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::getRegions()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::saveUser()
     * 
     * @return
     */
    public function saveUser() {
        ...
        $query = $this->_db->getQuery(true);
        $query->update($this->_db->quoteName('#__ksenmart_user_fields_values'));
        
        $f_set = '`value` = CASE `id`';
        foreach($f_values as $key => $value){
            $f_set .= ' WHEN '.$key.' THEN '.$this->_db->Quote($value);
        }
        $f_set .= ' ELSE `value` END';
        ...
        if($sendEmail == 'on') {
            KMUsers::setUserSubscribeGroup($user->id);
        } else {
            KMUsers::removeUserSubscribeGroup($user->id);
        }
    ...
    /**
     * KsenMartModelShopProfile::getOrders()
     * 
     * @return
     */
    public function getOrders() {
        ...
        for ($k = 0; $k < count($orders); $k++) {
            ...
            $orders[$k]->cost_val = KMPrice::showPriceWithTransform($orders[$k]->cost);
        }
    ...
    /**
     * KsenMartModelShopProfile::getOrder()
     * 
    ...
    public function getOrder($order_id = 0) {
        ...
        $order = KMOrders::getOrder($order_id);
        ...
        if (!empty($order)) {
            $order->items = KMOrders::getOrderItems($order_id);
        }
    ...
    /**
     * KsenMartModelShopProfile::getFavorities()
     * 
     * @return
     */
    public function getFavorities() {
        ...
        $rows           = KMSystem::getTableByIds($user->favorites, 'products', array('t.id'));

        if(!empty($rows)){
            foreach($rows as &$row){
                $row = KMProducts::getProduct($row->id);
            }
    ...
    public function removeFavorite($id) {
        ...
        if(!empty($id) && $id > 0){
            ...
            if(KMSystem::updateUser($user->id, 'favorites', $user->favorites)){
    ...
    public function removeWatched($id) {
        ...
        if(!empty($id) && $id > 0){
            ...
            if(KMSystem::updateUser($user->id, 'watched', $user->watched)){
    ...
    /**
     * KsenMartModelShopProfile::getWatched()
     * 
     * @return
     */
    public function getWatched() {
        ...
        $rows       = KMSystem::getTableByIds($user->watched, 'products', array('t.id'));
    ...
    /**
     * KsenMartModelShopProfile::addAddress()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::deleteAddress()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::setDefaultAddress()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::getAddresses()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::getTotal()
     * 
     * @return
     */
    function getTotal() {
    ...
    /**
     * KsenMartModelShopProfile::getPagination()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::getComments()
     * 
     * @return
     */
    ...
    /**
     * KsenMartModelShopProfile::setProductLinksOfReviews()
     * 
    ...
    function setProductLinksOfReviews($comments) {
        ...
        foreach ($comments as $comment) {
            $comment->link = JRoute::_('index.php?option=com_ksenmart&view=shopproduct&id=' . $comment->p_id . ':' . $comment->alias);
        }
    ...
    /**
     * KsenMartModelShopProfile::setProductImagesOfReviews()
     * 
     * @param mixed $comments
     * @return
     */
    function setProductImagesOfReviews($comments) {
        ...
        foreach ($comments as $comment) {
            $comment->small_img = KMMedia::resizeImage($comment->filename, $comment->folder, $params->get('thumb_width'), $params->get('thumb_height'));
        }
    ...
    /**
     * KsenMartModelShopProfile::getShippingTitle()
     * 
     * @param mixed $type_id
     * @return
     */
