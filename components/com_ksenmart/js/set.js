jQuery(document).ready(function(){
	...
	jQuery('#item .unit .to-fav a').click(function(){
		if (user_id==0)
		{
			jQuery('.popup').hide();	
		...
		else
		{
			...
			jQuery.ajax({
				...
				success:function(data){
					ShowMessage('<h2>Теперь этот товар у вас в избранных .</h2>');
				}
			});
		}
		return false;
	...
	jQuery('.price-set a').click(function(){
		if (user_id==0)
		{
			jQuery('.popup').hide();	
			jQuery(".form-login").fadeIn(400);
		...
		else
		{
			var prd_id=jQuery(this).attr('prd_id');
			jQuery.ajax({
				url:URI_ROOT+'index.php?option=com_ksenmart&task=shopajax.add_watched&id='+prd_id+'&tmpl=ksenmart',
				success:function(data){
					ShowMessage('<h2>Теперь вы следите за этим товаром .</h2>');
				}
			});
		}
		return false;
