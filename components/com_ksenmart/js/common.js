jQuery(document).ready(function(){        
	...
	jQuery('.buy [type="submit"]').parents('form').on('submit', function(){
		...
		if(!flag)
		{
			jQuery('body').animate({'scrollTop':form.offset().top},500); 
			return false;
		}	
		jQuery.ajax({
		...
		if(order_process == 1){
			jQuery.ajax({
				url: URI_ROOT+'index.php?option=com_ksenmart&view=shopopenorder&task=shopopenorder.get_order_id&tmpl=ksenmart',
				success: function(data){
					var order_id = data;
					if (order_id == 0){
						if (user_id == 0){
							KMOpenPopupWindow(URI_ROOT+'index.php?option=com_ksenmart&view=shopopenorder&'+form.serialize()+'&tmpl=component', 610, 300);
						}else{
							jQuery.ajax({
								url: URI_ROOT+'index.php?option=com_ksenmart&view=shopopencart&task=shopopencart.add_to_cart&layout=minicart&'+form.serialize()+'&tmpl=ksenmart',
								success: function( data ) {
	...
	jQuery('.buy-price .buy a').on('click',function(){
		...
		if (!flag)
		{
			jQuery('body').animate({'scrollTop':jQuery('#item').offset().top},500); 
			return false;
		}		
		KMOpenPopupWindow(URI_ROOT+'index.php?option=com_ksenmart&view=shopopenorder&'+form.serialize()+'&close_order=1&tmpl=component',920,Math.round(jQuery(window).height()*9/10));
		return false;
	});
		...
		if (order_process==1)
		{
			jQuery.ajax({
				...
				success: function( data ) {
					...
					if (order_id==0)
					{
						KMOpenPopupWindow(URI_ROOT+'index.php?option=com_ksenmart&view=shopopenorder&'+form.serialize()+'&tmpl=component',920,Math.round(jQuery(window).height()*9/10));	
					}
					else
					{
						jQuery.ajax({
	...
	jQuery('body').on('click', '.item-cart .del a, #minicart .remove a', function(){
		...
		jQuery.ajax({
			url:URI_ROOT+'index.php?option=com_ksenmart&view=shopopencart&layout=minicart&task=shopopencart.update_cart&item_id='+item_id+'&count='+count+'&tmpl=ksenmart',
			success: function(data){
...
function closeKMMessage(delay){
    ...
    jQuery('.km-message').fadeIn(500).delay(delay).fadeOut(500, function(){jQuery(this).remove()});
}
...
function KMShowPopup(parent,obj,rel_left,rel_top)
{
	...
	obj.css({'left':left-rel_left,'top':top-rel_top});
...
function KMOpenPopupWindow(url,width,height)
{
	...
	jQuery('#km-iframe-window').css({'width':width,'height':height,'margin-left':Math.round(-width/2),'margin-top':Math.round(-height/2)});
	jQuery('#km-iframe-window iframe').css({'width':width,'height':height,'border':'none'});
}
...
function KMGetLayouts(data){
	jQuery.ajax({
		url:URI_ROOT+'index.php?option=com_ksenmart&task=get_layouts&view=shopopencart&tmpl=ksenmart',
		data: data,
		dataType: 'json',
		async: false,
