...
class plgKMDiscountCountdown extends KMDiscountPlugin {
	...
	var $_params=array('first_value'=>0,'type'=>1,'period_value'=>0,'period_type'=>0,'last_value'=>0);
	...
	{	
		...
		foreach($discounts as $discount)
		{
			...
			if ($user_last_activity+700<time() || empty($user_last_activity) || empty($start_time))
			{	
	...
	function onSetCartDiscount($cart=null,$discount_id=null)
	{
		...
		foreach($cart->items as &$item)
		{
			...
			{
				case '0':
					$times=floor((time()-$start_time)/60);
					break;
				case '1':
					$times=floor((time()-$start_time)/3600);
					break;
				case '2':
					$times=floor((time()-$start_time)/86400);
					break;					
			}
			$discount->params['value']=$discount->params['first_value']-$discount->params['period_value']*$times;
