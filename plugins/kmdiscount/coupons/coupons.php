...
class plgKMDiscountCoupons extends KMDiscountPlugin {
	...
	var $_params=array('value'=>0,'type'=>1,'repeated'=>0,'mode'=>0,'coupons'=>array());
	...
	{	
		...
		{
			$query=$db->getQuery(true);
			$query->insert('#__ksenmart_discount_coupons')->columns(array('discount_id','code','published'))->values($discount_id.','.$db->quote($new_coupon).',1');
			$db->setQuery($query);
			$db->query();
		}
		return true;
	...
	{	
		...
		if ($task=='set_discount_coupon')
		{
			...
			$query=$db->getQuery(true);
			$query->select('kdc.discount_id,kdc.id,kd.params')->from('#__ksenmart_discount_coupons as kdc')
			->innerjoin('#__ksenmart_discounts as kd on kd.id=kdc.discount_id')
			->where('kdc.code='.$db->quote($code))->where('kdc.published=1')->where('kd.enabled=1');
			$db->setQuery($query);
		...
		foreach($discounts as $discount)
		{
			...
			if ($discount->params['mode']==1)
			{
				...
				if (empty($created))
				{
					...
					$query=$db->getQuery(true);
					$query->insert('#__ksenmart_discount_coupons')->columns(array('discount_id','code','published'))->values($discount->id.','.$db->quote($code).',1');
					$db->setQuery($query);
					$db->query();
	...
	function onAfterExecuteShopopencartCloseorder($model)
	{
		...
		if (!empty($coupon_id))
		{
			$db=JFactory::getDBO();
			$query=$db->getQuery(true);
			$query->select('kd.params')->from('#__ksenmart_discount_coupons as kdc')
			->innerjoin('#__ksenmart_discounts as kd on kd.id=kdc.discount_id')
			->where('kdc.id='.$coupon_id)->where('kdc.published=1')->where('kd.enabled=1');
			$db->setQuery($query);
	...
	function onGetDiscountContent($discount_id=null)
	{
		...
		if (!empty($created) && empty($coupon_id))
			return;			
		if (!empty($created))
			$content=str_replace('{code}',$created,$content);
		return $content;
	}
