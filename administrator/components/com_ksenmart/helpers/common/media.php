...
class KMMedia {
    ...
    public static function resizeImage($file, $folder, $width = 120, $height = 120, $iparams = array()) {
        ...
        if(!JFile::exists($dst_filename)) {
            if($file != '') {
                $src_filename = JPATH_ROOT . DS . 'media' . DS . 'ksenmart' . DS . 'images' . DS . $folder . DS . 'original' . DS . basename($file);
            } else {
                $src_filename = JPATH_ROOT . DS . 'media' . DS . 'ksenmart' . DS . 'images' . DS . $folder . DS . 'no.jpg';
            }
    ...
    public static function saveItemMedia($id = null, $data = array(), $owner_type = null, $folder = null) {
        ...
        if(isset($data['images']) && $data['images']) {
            foreach($data['images'] as $k => $v) {
                $k = (int)$k;
                if($v['task'] == 'save') {
					$table = JTable::getInstance('files', 'KsenmartTable', array());
    ...
    public static function deleteItemMedia($id = null, $owner_type = null) {
        ...
        foreach($medias as $media) {
            if($media->media_type == 'image') {
                KMMedia::deletePhoto($media->filename, $media->folder);
            }
            if($media->media_type == 'file') {
    ...
    public static function deletePhoto($filename, $folder) {
        $db = JFactory::getDBO();
        $subfolders = scandir(JPATH_ROOT . '/media/ksenmart/images/' . $folder);
        foreach($subfolders as $subfolder) {
            if($subfolder != '.' && $subfolder != '..' && is_dir(JPATH_ROOT . '/media/ksenmart/images/' . $folder . '/' . $subfolder)) {
                $files = scandir(JPATH_ROOT . '/media/ksenmart/images/' . $folder . '/' . $subfolder);
                foreach($files as $file) {
                    if($subfolder != '.' && $subfolder != '..' && strpos($file, $filename) === true) unlink(JPATH_ROOT . '/media/ksenmart/images/' . $folder . '/' . $subfolder . '/' . $file);
                }
            }
        }
        $query = $db->getQuery(true);
    ...
    public static function deleteFile($filename, $folder) {
        $db = JFactory::getDBO();
        $subfolders = scandir(JPATH_ROOT . '/media/ksenmart/files/' . $folder);
        foreach($subfolders as $subfolder) {
            if($subfolder != '.' && $subfolder != '..' && is_dir(JPATH_ROOT . '/media/ksenmart/files/' . $folder . '/' . $subfolder)) {
                $files = scandir(JPATH_ROOT . '/media/ksenmart/files/' . $folder . '/' . $subfolder);
                foreach($files as $file) {
                    if($subfolder != '.' && $subfolder != '..' && strpos($file, $filename) === true) unlink(JPATH_ROOT . '/media/ksenmart/files/' . $folder . '/' . $subfolder . '/' . $file);
                }
            }
        }
        $query = $db->getQuery(true);
