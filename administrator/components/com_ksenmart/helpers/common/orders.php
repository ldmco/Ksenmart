...
class KMOrders {
    public function getOrder($oid) {
        if(!empty($oid)) {
            ...
            $db->setQuery($query);
            $order = $db->loadObject();
            KMOrders::setUserInfoField2Order($order);
    ...
    private function setOrderItemsProperties(&$order, $oid) {
        if(!empty($order)) {
    ...
    public function getOrderItems($oid) {
        if(!empty($oid)) {
            ...
            if(!empty($order)) {
                KMOrders::setOrderItemsProperties($order, $oid);
                foreach($order as $o_item) {
                    $o_item->product = KMProducts::getProduct($o_item->product_id);
                    $o_item->val_price = KMPrice::showPriceWithTransform($o_item->price);
                }
            }
    ...
    public function setUserInfoField2Order(&$order) {
        if(!empty($order)) {
    ...
    public function sendOrderMail($order_id, $admin = false) {
		JRequest::setVar('id',$order_id);
        $model=KMSystem::getModel('orders');
        $order = $model->getOrder();
		KMOrders::setOrderItemsProperties($order, $order_id);
		if(!empty($order->address_fields)) {
			$order->address_fields = implode(', ', $order->address_fields);
		}		
    ...
    function getPrintformDate($date) {
        return date('d.m.Y', strtotime($date));
    }

    function getPrintformCustomerName($customer_fields) {
        $customer_name = '';
    ...
    function getPrintformCustomerAddress($address_fields) {
        $customer_address = '';
    ...
    public function getOrderField($oid, $field){
