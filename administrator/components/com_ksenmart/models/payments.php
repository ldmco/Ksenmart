...
class KsenMartModelPayments extends JModelKMAdmin {
	...
	function getListItems()
	{
		...
		$query=KMMedia::setItemMainImageToQuery($query,'payment','kp.');
    ...
    function getPayment($vars=array())
	{
		...
		$payment=KMSystem::loadDbItem($id,'payments');
		$payment=KMMedia::setItemMedia($payment,'payment');
	...
	function getPaymentParamsForm()
	{
		...
		$query=$this->db->getQuery(true);
		$query->select('enabled')->from('#__extensions')->where('element='.$this->db->quote($type))->where('folder="kmpayment"');
		$this->db->setQuery($query);
    ...
    function savePayment($data)
	{
		...
		$id = $table->id;
		KMMedia::saveItemMedia($id,$data,'payment','payments');
