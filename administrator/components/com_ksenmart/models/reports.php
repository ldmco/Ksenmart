...
class KsenMartModelReports extends JModelKMAdmin {
	...
	function getProducts()
	{
		...
		foreach($items as &$item)
			$item->img = KMMedia::resizeImage($item->filename,$item->folder,30,30,json_decode($item->params,true));
	...
	function getOrders()
	{
		...
		$query=$this->db->getQuery(true);
		$query->select('*')->from('#__ksenmart_orders')->where('date_add>'.$this->db->quote($from_date))->where('date_add<'.$this->db->quote($to_date));
		$this->db->setQuery($query);
		$total_orders=$this->db->loadObjectList();	
		...
		->where('o.date_add>'.$this->db->quote($from_date))->where('o.date_add<'.$this->db->quote($to_date))
