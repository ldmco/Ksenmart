...
class KsenMartModelComments extends JModelKMAdmin {
	...
	function getComment($vars=array())
	{
		...
		$comment=KMSystem::loadDbItem($id,'comments');
		if (isset($vars['user_id']))
			$comment->user_id=$vars['user_id'];			
		...
		$this->db->setQuery($query);
		$comment->rates=$this->db->loadObjectList('rate_id');		
	...
	function getRate()
	{
	   $this->onExecuteBefore('getRate');
		...
		$id=JRequest::getInt('id');
		$rate=KMSystem::loadDbItem($id,'commentrates');
