...
<tr class="list_item <?php echo ($this->item->published==0?'disabled':'')?> <?php echo ($this->item->parent_id!=0?'child_item child_item_'.$this->item->parent_id:'');?> <?php echo (isset($this->item->set_id)?'child_item child_item_'.$this->item->set_id:'');?>" >
	...
	<td class="name stretch"> 
		...
		<div class="descr">
			<a href="<?php echo KMProducts::generateProductLink($this->item->id); ?>" target="_blank"><?php echo $this->item->title; ?></a>
			<p>
	...
	<td class="price changeble">
		<span><?php echo KMPrice::showPriceWithoutTransform($this->item->price,$this->item->price_type)?></span>
		<p><?php echo KMPrice::showPriceWithoutTransform('<input type="text" name="items['.$this->item->id.'][price]" class="inputbox" value="'.$this->item->price.'">',$this->item->price_type)?></p>
	</td>
