...
class KMProducts {
    ...
    public static function getProduct($id) {
        ...
        if($row && !empty($row)) {
            ...
            $row->link  = JRoute::_('index.php?option=com_ksenmart&view=shopproduct&id=' . $row->id . ':' . $row->alias);
    ...
    public function generateProductLink($pid){
        if(!empty($pid) && $pid > 0){
            ...
            if(!empty($product)){
                return JRoute::_('index.php?option=com_ksenmart&view=shopproduct&id=' . $product->id . ':' . $product->alias);
            }
        }
        return null;
    }
