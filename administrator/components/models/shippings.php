...
class KsenMartModelShippings extends JModelKMAdmin {
	...
	function populateState()
	{
		...
		$methods=array_filter($methods,'KMFunctions::filterStrArray');
	...
	function getListItems()
	{
		...
		$query=KMMedia::setItemMainImageToQuery($query,'shipping','s.');		
	...
	function getShipping()
	{
		...
		$shipping=KMSystem::loadDbItem($id,'shippings');
		$shipping=KMMedia::setItemMedia($shipping,'shipping');
		...
		{
			$query = $this->db->getQuery(true);
			$query->select('*')->from('#__ksenmart_shipping_fields')->where('shipping_id='.(int)$shipping->id)->where('position='.$this->db->quote('customer'))->order('ordering');
			$this->db->setQuery($query);
			$shipping->user_fields = $this->db->loadAssocList('id');
			foreach($shipping->user_fields as &$user_field)
			{
				if ($user_field['type']=='select')
				{
					...
					$this->db->setQuery($query);
					$user_field['values']= $this->db->loadObjectList('id');			
				}
			}
			$query = $this->db->getQuery(true);
			$query->select('*')->from('#__ksenmart_shipping_fields')->where('shipping_id='.(int)$shipping->id)->where('position='.$this->db->quote('address'))->order('ordering');
			$this->db->setQuery($query);
			$shipping->address_fields = $this->db->loadAssocList('id');
			foreach($shipping->address_fields as &$address_field)
			{
				if ($address_field['type']=='select')
				{
					...
					$this->db->setQuery($query);
					$address_field['values']= $this->db->loadObjectList('id');			
				}
			}
		}
	...
	function getShippingParamsForm()
	{
		...
		$query=$this->db->getQuery(true);
		$query->select('enabled')->from('#__extensions')->where('element='.$this->db->quote($type))->where('folder="kmshipping"');
		$this->db->setQuery($query);
	...
	function SaveShipping($data)
	{
		...
		foreach($data['regions'] as &$country)
			$country=array_filter($country,'KMFunctions::filterArray');
		unset($country);	
		...
		$id = $table->id;
		KMMedia::saveItemMedia($id,$data,'shipping','shippings');
