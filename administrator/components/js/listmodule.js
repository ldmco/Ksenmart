var KMListModule = function(variables){
	...
	this.setListModuleClose = function(){
		var Module=this;	
		jQuery('.'+Module.module+' .km-list-left-module-title .sh').live('click',function(){
	...
	this.setListModuleHover = function(){
		var Module=this;
		jQuery('.'+Module.module+' label').live('mouseover',function(){
			jQuery(this).find('p').css('visibility','visible');
		});
		jQuery('.'+Module.module+' label').live('mouseout',function(){
	...
	this.setListModuleDelete = function(){
		var Module=this;
		jQuery('.'+Module.module+' .delete').live('click',function(){
	...
	this.refresh = function(){
		var Module=this;
		jQuery.ajax({
			url:'index.php?option=com_ksenmart&view='+Module.view+'&task=update_module&module='+Module.module,
			dataType:'json',
			async:false,
			success:function(responce){
				if (responce.errors == 0)
