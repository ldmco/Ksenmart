...
class KsenMartControllerUsers extends KsenMartController
{
	...
	function set_user_subsriber()
	{
		...
		if ($value==1 && !in_array(KMUsers::getSubscribersGroupID(),$groups))
		{
			$query=$db->getQuery(true);
			$values=array(KMUsers::getSubscribersGroupID(),$user_id);
		...
		{
			$query = $db->getQuery(true);
			$query->delete('#__user_usergroup_map')->where('user_id='.$user_id)->where('group_id='.KMUsers::getSubscribersGroupID());
			$db->setQuery($query);
			$db->query();			
		}	
