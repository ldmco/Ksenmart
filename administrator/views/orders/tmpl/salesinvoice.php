...
<table cellpadding="0" cellspacing="0" border="0" class="mainTable_normal" width="100%">
	<tr>
		<td colspan=2><b>СЧЁТ-ФАКТУРА № <span class="inline_edit underlined"><?php echo $this->order->id?></span> от <span class="inline_edit underlined"><?php echo KMOrders::getPrintformDate($this->order->date_add)?> г.</span></b></td>
		<td>(1)</td>
	</tr>
	<tr>
		<td colspan=2><b>ИСПРАВЛЕНИЕ № <span class="inline_edit underlined"><?php echo $this->order->id?></span> от <span class="inline_edit underlined"><?php echo KMOrders::getPrintformDate($this->order->date_add)?> г.</span></b></td>
		<td>(1а)</td>
	</tr>
	<tr>
		<td colspan=3>&nbsp;</td>
	...
	<tr>
		...
		<td class="leftAlign underlined"><b class="inline_edit">
			<?php echo KMOrders::getPrintformCustomerName($this->order->customer_fields);?>&nbsp;
			<?php echo KMOrders::getPrintformCustomerAddress($this->order->address_fields);?>&nbsp;
	...
	<tr>
		...
		<td class="leftAlign underlined"><b class="inline_edit"><?php echo KMOrders::getPrintformCustomerName($this->order->customer_fields);?></b></td>
		<td>(6)</td>
	</tr>
	<tr>
		<td class=leftAlign>Адрес:</td>
	...
	<tr>
		...
		<td class="leftAlign underlined"><b class="inline_edit"><?php echo KMPrice::getCurrencyName()?></b>, <b class="inline_edit"><?php echo KMPrice::getCurrencyCode()?></b></td>
		<td>(7)</td>
	</tr>
	<tr>
		<td colspan=3>&nbsp;</td>
...
<table width="100%" border="0" cellpadding=0 cellspacing=0
	...
	<tr>
		...
		<td class="b_left b_top rightAlign nobr"><?php echo KMPrice::showPriceWithoutTransform($item->price)?></td>
		<td class="b_left b_top rightAlign nobr"><?php echo KMPrice::showPriceWithoutTransform($item->price*$item->count*(100-$this->params->get('printforms_nds'))/100)?></td>
		<td class="b_left b_top rightAlign">0,00</td>
		<td class="b_left b_top rightAlign"><?php echo $this->params->get('printforms_nds')?>%</td>
		<td class="b_left b_top rightAlign"><?php echo KMPrice::showPriceWithoutTransform($item->price*$item->count*$this->params->get('printforms_nds')/100)?></td>
		<td class="b_left b_top rightAlign nobr"><?php echo KMPrice::showPriceWithoutTransform($item->price*$item->count)?></td>
	...
	<tr>
		<td class="b_left b_top leftAlign">доставка <?php echo KMShipping::getShippingName($this->order->shipping_id)?></td>
	...
	<tr class=totals>
		...
		<table class=mainTable_normal border="0" cellpadding=0 cellspacing=0 width="100%">
			<tr>
				...
				<td class="rightAlign nobr"><b><?php echo KMFunctions::stringView($this->order->cost)?>, <?php echo ($this->params->get('printforms_nds')!=0?'в т.ч. НДС':'без налога (НДС)')?></b></td>
			</tr>
		</table>
		</td>
		<td class="b_left b_top b_bottom" colspan="2">&times;</td>
		<td class="b_left b_top b_bottom rightAlign"><?php echo KMFunctions::stringView($this->order->cost*$this->params->get('printforms_nds')/100)?></td>
		<td class="b_left b_top b_bottom rightAlign nobr"><?php echo KMFunctions::stringView($this->order->cost*(100-$this->params->get('printforms_nds'))/100)?></td>
		<td class="b_left b_top">&nbsp;</td>
		<td class="b_top">&nbsp;</td>
		<td class="b_top">&nbsp;</td>
	</tr>
