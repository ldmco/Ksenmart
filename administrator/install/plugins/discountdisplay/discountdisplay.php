	...
	function onBeforeExecuteShopopencartCloseorder($model) {	
        ...
        foreach($cart->items as $item) {
            if(isset($item->discounts)) {
                foreach($item->discounts as $discount_id => $discount) {
                    if($discount->sum) {
                        if(!isset($discounts[0])) $discounts[0] = array('discount_value' => 0, 'discounts' => array());
                        $discounts[0]['discount_value'] += $discount->discount_value;
                        $discounts[0]['discounts'][$discount_id] = $discount->params;
                    } else {
                        if(!isset($discounts[$discount_id])) $discounts[$discount_id] = array('discount_value' => 0, 'discounts' => array($discount_id => $discount->params));
