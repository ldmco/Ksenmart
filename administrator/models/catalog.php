...
class KsenMartModelCatalog extends JModelKMAdmin {
    ...
    function getListItems() {
        ...
        if(!empty($searchword)) $query->where('p.title like ' . $this->db->quote('%' . $searchword . '%') . ' or p.product_code like ' . $this->db->quote('%' . $searchword . '%'));
    ...
    function copyListItems($products, $parent_id = 0, $childs_group = 0) {
        ...
        foreach($products as $product) {
            ...
            $table->title .= ' (1)';
            $table->alias = KMFunctions::GenAlias($table->title);
    ...
    function getSet($categories = array()) {
        ...
        if(count($ids)) {
            ...
            $query = $this->db->getQuery(true);
            $query->select('p.*')->from('#__ksenmart_products as p')->innerjoin('#__ksenmart_products_relations as pr on pr.relative_id=p.id')->where('pr.relation_type=' . $this->db->quote('set'))->where('pr.product_id=' . $id);
            $query = KMMedia::setItemMainImageToQuery($query);
            $this->db->setQuery($query);
            $set->relative = $this->db->loadObjectList('id');
        }
    ...
    function getProduct($categories = array()) {
        ...
        if($product->is_parent == 1) {
            ...
            foreach($product->childs as &$child) {
                ...
                foreach($child->products as &$prd) $prd->small_img = KMMedia::resizeImage($prd->filename, $prd->folder, $this->params->get('admin_product_thumb_image_width', 36), $this->params->get('admin_product_thumb_image_heigth', 36), json_decode($prd->params, true));
            }
        }

        $query = $this->db->getQuery(true);
        $query->select('p.*')->from('#__ksenmart_products as p')->innerjoin('#__ksenmart_products_relations as pr on pr.relative_id=p.id')->where('pr.relation_type=' . $this->db->quote('relation'))->where('pr.product_id=' . $id);
        $query = KMMedia::setItemMainImageToQuery($query);
        $this->db->setQuery($query);
        $product->relative = $this->db->loadObjectList('id');
        foreach($product->relative as &$prd) $prd->small_img = KMMedia::resizeImage($prd->filename, $prd->folder, $this->params->get('admin_product_thumb_image_width', 36), $this->params->get('admin_product_thumb_image_heigth', 36), json_decode($prd->params, true));
    ...
    function saveCategory($data) {
        ...
        $data['alias'] = KMFunctions::CheckAlias($data['alias'], $data['id']);
        if($data['alias'] == '') $data['alias'] = KMFunctions::GenAlias($data['title']);
