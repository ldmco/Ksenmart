	...
	function getImportInfo()
	{
		...
		for ($k=0; $data=fgetcsv($f,10000,";"); $k++)
		{
			...
			if ($k>0)
			{
				...
				$this->db->setQuery($query);
				$def_price_type=$this->db->loadResult();
				
				$query=$this->db->getQuery(true);
				$query->select('id')->from('#__ksenmart_product_units');
				$this->db->setQuery($query,0,1);
				$def_unit=$this->db->loadResult();						
				...
				if (isset($product_data['price_type']) && $product_data['price_type']!='')
				{
					...
					->where('title='.$this->db->quote($product_data['price_type']));
					$this->db->setQuery($query);
					$price_type=$this->db->loadObject();
				...
				if (count($product)==0)
				{
					...
					$values=array(
						'parent_id'=>$product_data['parent'],
						'childs_group'=>$product_data['childs_group'],
						'title'=>$this->db->quote($product_data['title']),
						'alias'=>$this->db->quote($alias),
						'price'=>$product_data['price'],
						'old_price'=>$product_data['old_price'],
						'price_type'=>$product_data['price_type'],
						'in_stock'=>$product_data['in_stock'],
						'product_code'=>$this->db->quote($product_data['product_code']),
						'product_packaging'=>$product_data['product_packaging'],
						'product_unit'=>$product_data['product_unit'],
						'content'=>$this->db->quote($product_data['content']),
						'promotion'=>$product_data['promotion'],
						'manufacturer'=>$product_data['manufacturer'],
						'published'=>1,
						'metatitle'=>$this->db->quote($product_data['metatitle']),
						'metadescription'=>$this->db->quote($product_data['metadescription']),
						'metakeywords'=>$this->db->quote($product_data['metakeywords']),
						'date_added'=>'NOW()',
						'type'=>$this->db->quote($product_data['type'])
					);
				...
				else
				{
					...
					if (isset($product_data['manufacturer']))
						$to_update[]='manufacturer='.$this->db->quote($product_data['manufacturer']);	
					if (isset($product_data['price_type']))
						$to_update[]='price_type='.$this->db->quote($product_data['price_type']);		
					if (isset($product_data['product_unit']))
						$to_update[]='product_unit='.$this->db->quote($product_data['product_unit']);							
							...
							case 'text':
								if ($product_data['property_'.$property->id]!='')
								{
									...
									if (count($prop_value)==0)
									{
										...
										$qvalues=array($property->id,$this->db->quote($product_data['property_'.$property->id]),$this->db->quote($alias));
										$query=$this->db->getQuery(true);
									...
									$this->db->setQuery($query);
									$prod_prop_value=$this->db->loadObject();
								...
								foreach($prop_vals as $prop_val)
								{
									if ($prop_val!='')
									{
										...
										if (count($prop_value)==0)
										{
											...
											$qvalues=array($property->id,$this->db->quote($prop_val),$this->db->quote($alias));
											$query=$this->db->getQuery(true);
											$query->insert('#__ksenmart_property_values')->columns('property_id,title,alias')->values(implode(',', $qvalues));					
											$this->db->setQuery($query);
											$this->db->query();	
		...
		foreach($relatives as $product_id=>$relative)
		{
			...
			foreach($relative as $relative_title)
			{
				...
				{
					$query=$this->db->getQuery(true);
					$query->select('id')->from('#__ksenmart_products')->where('title like '.$this->db->quote($relative_title));	
					$this->db->setQuery($query);
					$relative_id=$this->db->loadResult();
					if (!empty($relative_id))
					{
						$query=$this->db->getQuery(true);
						$query->select('*')->from('#__ksenmart_products_relations')->where('product_id='.$product_id)->where('relative_id='.$relative_id)->where('relation_type='.$this->db->quote('relation'));						
						$this->db->setQuery($query);
						$db_rel=$this->db->loadObject();	
						if (count($db_rel)==0)
						{						
							$qvalues=array($product_id,$relative_id,$this->db->quote('relation'));
							$query=$this->db->getQuery(true);
