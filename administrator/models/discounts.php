...
class KsenMartModelDiscounts extends JModelKMAdmin {
	...
	function populateState()
	{
		...
		$types=array_filter($types,'KMFunctions::filterStrArray');
	...
	function getListItems()
	{
		...
		$query->select('SQL_CALC_FOUND_ROWS d.*,e.name as plugin_name')->from('#__ksenmart_discounts as d')
		->leftjoin('#__extensions as e on e.element=d.type and e.folder="kmdiscount"')
	...
	function getDiscount()
	{
		...
		$discount=KMSystem::loadDbItem($id,'discounts');
		$discount=KMMedia::setItemMedia($discount,'discount');
	...
	function getDiscountParamsForm()
	{
		...
		$results = $dispatcher->trigger('onDisplayParamsForm',array($type,$params,$discount_id));
	...
	function SaveDiscount($data)
	{
		...
		$id = $table->id;
		KMMedia::saveItemMedia($id,$data,'discount','discounts');
