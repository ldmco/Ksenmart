...
*/
    ...
    public static function sendOrderIndoCustomer($order_id) {
        ...
        foreach ($order->items as &$item) {
            ...
            foreach ($properties as $property) {
                ...
                if (count($prop) > 0) {
                    ...
                    if ($prop->type == 'select' || $prop->type == 'radio') {
                        ...
                        if (count($val) > 0) $item_properties[count($item_properties) - 1]->value = $val->
                                title . ' ' . $prop->finishing;
                    }
                }
            }
            ...
            $link = JURI::root() . JRoute::_('index.php?option=com_ksenmart&view=shopproduct&id=' .
                $item->product->id . ":" . $item->product->alias . '&Itemid=' . $Itemid);
            $content .= '		
			<tr class="row_odd">
				<td class="vid_produkt">
					<a class="title_lookp" href="' . $link . '" >' . $item->product->title .
                '</a>
			';
            if ($item->product->product_code != '') $content .= ' <i>Арт. ' . $item->
                    product->product_code . '</i>';
            foreach ($item_properties as $item_property) {
                if (!empty($item_property->value)) $content .= '<br><span>' . $item_property->
                        title . ':</span> ' . $item_property->value;
            ...
            $content .= '							
				...
				<td align="center">' . KMPrice::showPriceWithTransform($item->price) .
                '</td>
				<td align="center" nowrap="nowrap">
					' . KMPrice::showPriceWithTransform($item->count * $item->price) . '
				</td>
			</tr>';
        }
        ...
        $content .= '
			<tr>
				...
				<td align="center"></td>
				<td></td>
				<td id="cart_total" align="center">' . KMPrice::
            showPriceWithTransform($order->cost) . '</td>
			</tr>
		';
        ...
        $content .= '
			<tr>
				...
				<td></td>
				<td id="cart_total" align="center">' . KMPrice::
            showPriceWithTransform($order->delivery_cost) . '</td>
			</tr>
			<tr>
				<td id="cart_total_label">
					Итого
				...
				<td></td>
				<td id="cart_total" align="center">' . KMPrice::
            showPriceWithTransform($order->total_cost + $order->delivery_cost) . '</td>
			</tr>
		</table>';
