...
class KsenMartControllerCatalog extends KsenMartController {
    ...
    function get_childs() {
        ...
        foreach ($items as $item) {
            $item->small_img = KMMedia::resizeImage($item->filename, $item->folder, $model->params->get('admin_product_thumb_image_width',36), $model->params->get('admin_product_thumb_image_heigth',36), json_decode($item->params, true));
            $item->medium_img = KMMedia::resizeImage($item->filename, $item->folder, $model->params->get('admin_product_medium_image_width',120), $model->params->get('admin_product_medium_image_heigth',120), json_decode($item->params, true));
