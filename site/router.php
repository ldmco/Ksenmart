...
function KsenMartBuildRoute( &$query )
{
	...
	if (!isset($query['view']))
		$query['view']='shopcatalog';		
		
	if (isset($query['task']) && isset($query['tmpl']) && $query['task']=='shopcatalog.filter_products' && $query['tmpl']=='ksenmart')
	{
		unset($query['task']);	
		unset($query['tmpl']);
	}
	...
	switch($query['view'])
	{	
		case 'shopopencart':$segments[]='cart';break;
		case 'shopprofile':
			$segments[]='profile';
			...
			if (isset($query['properties']))
			{
				...
				if (!empty($query['properties']) && is_array($query['properties']))
				{
					foreach($query['properties'] as $property)
					{
						...
						if (!empty($property_value->alias))
							$properties[$property_value->property_id][]=$property_value->alias;
						else	
							$properties[$property_value->property_id][]='propertyvalue-'.$property;
					}	
					foreach($properties as $key=>$property_values)
					{
						...
						$alias=$db->loadResult();	
						if (!empty($alias))
							$segments[]=$alias.'='.implode('+',$property_values);
						else	
...
function KsenMartParseRoute( $segments )
{
	...
	$vars['view']='shopcatalog';
		
	foreach($segments as $segment)
	{
		switch($segment)
		{
			case 'cart':$vars['view']='shopopencart';break;
			case 'profile':
				$vars['view']='shopprofile';
				if (isset($segments[1]))
					$vars['layout']=$segments[1];
				break;
			case 'reviews':
				$vars['view']='shopcomments';
				if (isset($segments[1]))
					$vars['id']=$segments[1];
				break;
			default:
