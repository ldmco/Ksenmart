...
class KsenMartModelShopOpenCart extends JModelKMAdmin {
    ...
    public function getShippings() {
        ...
        if(!empty($this->_region_id)){
            ...
            foreach($rows as $row) {
                $row->icon    = !empty($row->filename)?KMMedia::resizeImage($row->filename, $row->folder, 40, 20, json_decode($row->params_f, true)):'';
    ...
    private function getFieldsOrder($position, $type){
        ...
        if(!empty($position)){
            ...
            if($fields){
                ...
                KMOrders::updateOrderFields($this->order_id, $data);
    ...
    public function getOrderInfo() {
        ...
        if(!empty($this->order_id)) {
			$order = KMOrders::getOrder($this->order_id);
    ...
    public function getCart() {
        ...
        if(!empty($this->order_id)){
            $cart        = KMOrders::getOrder($this->order_id);
            $this->setDefaultCartValues($cart);
            $cart->items = KMOrders::getOrderItems($this->order_id);
            
            for($k = 0; $k < count($cart->items); $k++) {
                $cart->items[$k]->del_link   = JRoute::_('index.php?option=com_ksenmart&view=shopopencart&task=shopopencart.update_cart&item_id=' . $cart->items[$k]->id . '&count=0&Itemid=' . $Itemid);
        ...
        $cart->products_sum_val = KMPrice::showPriceWithTransform($cart->products_sum);
    ...
    public function addToCart() {
        ...
        if(count($prd) > 0 && ($count <= $prd->in_stock || $this->params->get('use_stock', 1) == 0)) {
            ...
            if($prd->type == 'set') {
                $related         = KMProducts::getSetRelated($id);
    ...
    public function updateCart() {
        ...
        $item       = KMSystem::getTableByIds(array($item_id), 'order_items', array(
            't.id',
            't.order_id',
            't.price',
            't.count',
        ...
        $prd = KMProducts::getProduct($item->product_id);
        $item_properties = array();
    ...
    public function closeOrder() {
        ...
        if(!empty($customer_fields['email'])) {
            KMOrders::sendOrderMail($this->order_id);
        }
        KMOrders::sendOrderMail($this->order_id, true);		
