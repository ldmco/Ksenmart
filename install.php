...
class com_ksenmartInstallerScript
{
	...
	function postflight($type,$parent){
		...
		self::KMinstallModule('Ksenmart - Фильтры','mod_km_filter',8,'left','{"layout":"_:default","moduleclass_sfx":"filter","cache":"1","cache_time":"900","cachemode":"static"}');
		self::KMinstallModule('Ksenmart - Контакты','mod_km_contacts',1,'head_block_2','{"Itemid":"","layout":"_:default","moduleclass_sfx":"","cache":"1"}');
		self::KMinstallModule('Ksenmart - Простой поиск','mod_km_simple_search',1,'content_top','{"layout":"_:default","count_result":"5","count_relevants":"3","count_categories":"1","count_manufactured":"1"}');
		self::KMinstallModule('Ksenmart - Отзывы','mod_km_shop_reviews',9,'left','{"count_review":"10","count_symbol":"200","layout":"_:default","moduleclass_sfx":"","cache":"1"}');
		self::KMinstallModule('Ksenmart - Профиль','mod_km_profile_info',11,'left','{"layout":"_:default"}',0,0);
		self::KMinstallModule('Ksenmart - Подписка','mod_km_subscribe',1,'content_bottom','{"layout":"_:default"}');
		self::KMinstallModule('Ksenmart - Категории','mod_km_categories',4,'left','{"layout":"_:dropdown","moduleclass_sfx":"","cache":"1","cache_time":"900","cachemode":"static"}');
		self::KMinstallModule('Ksenmart - Миникорзина','mod_km_minicart',1,'head_block_3','{"layout":"_:default"}');
		self::KMinstallModule('Ksenmart - Доставка','mod_km_shipping',10,'left','{"layout":"_:default"}');
		self::KMinstallModule('Ksenmart - Список продуктов','mod_km_products_list',3,'content_top','{"layout":"_:default","col":"10","type":"id"}');
		
		self::KMinstallModule('Ksenmart - Форма входа','mod_login',1,'head_block_2','{"pretext":"","posttext":"","login":"","logout":"","greeting":"1","name":"0","usesecure":"0","layout":"_:ksenmart","moduleclass_sfx":"","cache":"0"}');
		self::KMinstallModule('Ksenmart - Хлебные крошки','mod_breadcrumbs',2,'content_top','{"showHere":"0","showHome":"1","homeText":"","showLast":"1","separator":"\u2192","layout":"_:default","moduleclass_sfx":"","cache":"1","cache_time":"900","cachemode":"itemid"}');
		...
		if (empty($subscribers))
		{
			...
			$query=$db->getQuery(true);
			$query->insert('#__usergroups')->columns(implode(',',array_keys($values)))->values(implode(',',$values));
			$db->setQuery($query);
			$db->query();	
			self::KMRebuildUsergroups();
		}
		
		$db=JFactory::getDBO();
		$query=$db->getQuery(true);
		$params='{"shop_name":"","shop_email":"","shop_phone":"","modules_styles":"1","admin_product_limit":"40","calculate_set_price":"0","admin_product_thumb_image_width":"36","admin_product_thumb_image_heigth":"36","admin_product_medium_image_width":"120","admin_product_medium_image_heigth":"120","watermark":"0","watermark_image":"","watermark_type":"0","watermark_valign":"middle","watermark_halign":"center","displace":"1","valign":"middle","halign":"center","background_type":"color","background_file":"","background_color":"ffffff","show_products_from_subcategories":"1","show_out_stock":"1","order_process":"0","show_comment_form":"1","site_product_limit":"15","site_use_pagination":"1","parent_products_template":"list","include_jquery":"1","include_css":"1","only_auth_buy":"0","use_stock":"0","catalog_mode":"0","path_separator":" - ","thumb_width":"200","thumb_height":"200","middle_width":"350","middle_height":"350","mini_thumb_width":"110","mini_thumb_height":"110","count_result":"5","count_relevants":"3","count_categories":"1","count_manufactured":"1","count_symbol":"400","printforms_companyname":"","printforms_companyaddress":"","printforms_companyphone":"","printforms_nds":"","printforms_ceo_name":"","printforms_buh_name":"","printforms_bank_account_number":"","printforms_inn":"","printforms_kpp":"","printforms_bankname":"","printforms_bank_kor_number":"","printforms_bik":"","printforms_ip_name":"","printforms_ip_registration":"","printforms_company_logo":"","printforms_congritulation_message_template":"<p>\u0412\u0430\u0448\u0435\u043c\u0443 \u0437\u0430\u043a\u0430\u0437\u0443 \u043f\u0440\u0438\u0441\u0432\u043e\u0435\u043d \u043d\u043e\u043c\u0435\u0440 %s<\/p><p>\u041d\u0430\u0448\u0438 \u043c\u0435\u043d\u0435\u0434\u0436\u0435\u0440\u044b \u0441\u0432\u044f\u0436\u0443\u0442\u0441\u044f \u0441 \u0432\u0430\u043c\u0438 \u0432 \u0442\u0435\u0447\u0435\u043d\u0438\u0438 2 \u0447\u0430\u0441\u043e\u0432.<\/p><p>\u0422\u0430\u043a \u0436\u0435 \u0432\u044b \u043c\u043e\u0436\u0435\u0442\u0435 \u0443\u0437\u043d\u0430\u0442\u044c \u043e \u0441\u0442\u0430\u0442\u0443\u0441\u0435 \u0437\u0430\u043a\u0430\u0437\u0430 \u043f\u043e \u0442\u0435\u043b\u0435\u0444\u043e\u043d\u0443: 8 800 2000 600<\/p>"}';
	...
	function KMRebuildUsergroups($parent_id = 0, $left = 0)
	{
		$db=JFactory::getDBO();
		
		$query=$db->getQuery(true);
		$query->select('id')->from('#__usergroups')->where('parent_id='.(int)$parent_id)->order('parent_id, title');
		$db->setQuery($query);
		$ids=$db->loadColumn();
		...
		{
			$child =$ids[$i];
			$right = self::KMRebuildUsergroups($child, $right);
		...
		$query=$db->getQuery(true);
		$query->update('#__usergroups')->set('lft='.(int)$left)->set('rgt='.(int)$right)->where('id='.(int)$parent_id);
		$db->setQuery($query);
		if (!$db->query()) {
			return false;
		}
