	...
	function importCSV() {
		...
		for ($k = 0; $data = fgetcsv($f, 10000, ";"); $k++) {
			...
			if ($k > 0) {
				...
				if (isset($_POST['purchase_price']) && $_POST['purchase_price'] != '') $product_data['purchase_price'] = (float) str_replace(' ', '', $this->encode($data[$_POST['purchase_price']]));
				...
				if (isset($_POST['photos']) && $_POST['photos'] != '') $product_data['photos'] = trim($this->encode($data[$_POST['photos']]));
				...
				if (count($product) == 0) {
					...
					if (isset($product_data['purchase_price'])) $values['purchase_price'] = $db->quote($product_data['purchase_price']);
					...
					if (isset($product_data['purchase_price'])) $to_update[] = 'purchase_price=' . $db->quote($product_data['purchase_price']);
					...
					if (isset($product_data['photos']) && ($product_data['photos'] != '' || $product_data['photos'] == 'delete')) {
						$query = $db->getQuery(true);
				...
				if ($select_flag) {
					$query = $db->getQuery(true);
					$query->update('#__ksenmart_products')->set('select_properties=1')->where('id=' . (int)$product_id);
					$db->setQuery($query);
					$db->execute();
				}
			}
