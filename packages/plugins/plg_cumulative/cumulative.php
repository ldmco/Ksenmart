...
class plgKMDiscountCumulative extends KMDiscountPlugin
{
	...
	function onSetProductDiscount($prd = null, $discount_id = null)
	{
		...
		if (JFactory::getUser()->id != 0)
		{
			...
			$query->select('sum(cost)')->from('#__ksenmart_orders')->where('user_id=' . JFactory::getUser()->id)->where('status_id=5');
			$this->db->setQuery($query);
			$orders_sum = $this->db->loadResult();
		} else
			return $prd;
		...
		if ($need_key != -1)
		{
			$discount       = $this->calculateDiscountProduct($prd->price, $discount->params[$need_key]);
