...
require_once(JPATH_ROOT.'/plugins/system/ksencore/core/helpers/common/system.php');
...
class JFormFieldKSModules extends JFormField {
	...
	public function getInput() {
		...
		$query->select('a.id, a.title, a.position, a.published, map.menuid')
			->from('#__modules AS a')
			->join('LEFT', sprintf('#__modules_menu AS map ON map.moduleid = a.id AND map.menuid IN (0, %1$d, -%1$d)', KSSystem::getShopItemid()))
    ...
    protected function getCatOptions() {
        $options = array(JHtml::_('select.option', 0, JText::_('JALL')), JHtml::_('select.option', -1, JText::_('ksm_plugin_modules_noone')));
