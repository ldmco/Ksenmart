...
class KsenControllerAdmin extends JControllerAdmin
{
	...
	public function addtoken()
	{
		...
		$token  = $jinput->getString('shop_token');
		if (empty($token)) return;

		$db    = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->update('#__ksenmart_info')->set('token=' . $db->q($token))->where('package=' . $db->q('ksenmart'));
		$db->setQuery($query);
		$db->execute();

		$return_url = JRoute::_('/administrator/index.php?option=com_ksen&extension=com_ksenmart');
	...
	{
		$jinput   = JFactory::getApplication()->input;
		$mvc      = $jinput->getCmd('mvc', $jinput->getCmd('view', null));
		$formname = $jinput->getCmd('formname', $jinput->getCmd('layout', null));
		$close    = $jinput->getInt('close', 1);
