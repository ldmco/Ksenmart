...
class JFormFieldImages extends JFormField {
	...
	public function getInput() {
		...
		$document->addScript(JUri::base() . 'components/com_ksen/assets/js/upload/upload.js');
		...
		if(count($keys) == 0 || $keys[0] > 0) {
			...
			//$html .= '<a class="fileinput-button">';
			$html .= '	<span>' . JText::_('KS_UPLOAD') . '</span>';
			$html .= '	<input id="fileupload" type="file" name="files" multiple>';
			$html .= '</a>';
			//$html .= '<a class="btn add add_image">' . JText::_('KS_UPLOAD') . '</a>';
			$html    = KSSystem::wrapFormField('slidemodule', $this->element, $html);
			$html .= '<div class="images-drag-block">';
			$html .= '</div>';
			$html .= '<div class="images-drag-text">' . JText::_('KS_DRAG_IMAGES') . '</div>';
		}
