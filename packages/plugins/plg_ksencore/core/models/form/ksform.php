...
class JKSForm extends JForm {
	...
	public static function getInstance($name, $data = null, $options = array(), $replace = true, $xpath = false) {
		...
		// Это раньше работало? О_о
		//$dispatcher->trigger('onBeforeGet' . strtoupper($ext_prefix) . 'Form' . ucfirst($formname), array(&$this, &$name, &$data, &$options, &$replace, &$xpath));
		$dispatcher->trigger('onBeforeGet' . strtoupper($ext_prefix) . 'Form' . ucfirst($formname), array(&$name, &$data, &$options, &$replace, &$xpath));
		...
		$dispatcher->trigger('onAfterGet' . strtoupper($ext_prefix) . 'Form' . ucfirst($formname), array(&$name, &$forms[$name]));
