...
class KSUsers
{
	...
	public static function addUser($data)
	{
		...
		if (empty($data['username']) || empty($data['password1'])
			|| empty($data['password2']) || empty($data['email1']))
		{
			return false;
		}
		...
		{
			include(JPATH_ROOT . '/components/com_users/models/registration.php');
		}

		$model = UsersModelRegistration::getInstance('registration', 'UsersModel');
		...
		$db = JFactory::getDBo();
		$query = $db->getQuery(true);
		$query->select('id')->from('#__users')->where('username=' . $db->q($data['username']));
		$db->setQuery($query);
		$id = $db->loadResult();
		/*$table = JTable::getInstance('user', 'JTable');
		$table->load(['email1' => $data['email1']]);*/
		...
		if ($id)
		{
			...
			if (!empty($data['phone']))
			{
				$qvalues['phone'] = $db->q($data['phone']);

				$query = $db->getQuery(true);
				$query->update('#__ksen_users')->set('phone=' . $qvalues['phone'])->where('id=' . $id);
