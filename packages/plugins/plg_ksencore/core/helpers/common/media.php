...
class KSMedia {
	...
	public static function resizeImage($file, $folder, $width = 120, $height = 120, $iparams = array(), $loc_ext_name_com = null) {
		...
		if (!isset($iparams['autosize'])) $iparams['autosize'] = false;
		...
		if (!JFile::exists($dst_filename)) {
			...
			if ($image->get_width() > $width || $image->get_height() > $height) {
				...
				if ($iparams['displace'] == 0) {
					...
					if ($ratio_in > $ratio_out) {
						$image->resize((int) $height * $ratio_in, $height);
					} else {
			...
			$image->background($width, $height, $iparams, $bg_file, $params->get('background_color', ''));
	...
	public static function setItemMedia($item = null, $owner_type = null, $extension = '') {
		if (!$item) return false;

		global $ext_name, $ext_name_com;
		if (empty($extension)) $extension = $ext_name;
		$item->images = array();
		$item->files  = array();
		$item->videos = array();
		...
		$query->select('*')->from('#__' . $extension . '_files')->where('owner_type=' . $db->quote($owner_type))->where('owner_id=' . $owner_id)->order('ordering');
		$db->setQuery($query);
	...
	public static function saveItemMedia($id = null, $data = array(), $owner_type = null, $folder = null) {
		...
		if (isset($data['files']) && $data['files']) {
			foreach ($data['files'] as $k => $v) {
				...
				$v['owner_type'] = $owner_type;
				$v['media_type'] = 'file';
				$v['mime_type']  = mime_content_type(JUri::root().'media/'.$ext_name_com.'/files/'.$folder.'/'.$v['filename']);
				$v['folder']     = $folder;
				if ($k > 0) {
					$v['id'] = $k;
				}
		...
		$query
			...
			->where($db->qn('owner_id') . '=' . $db->q($owner_id))
			->where($db->qn('owner_type') . '=' . $db->q($owner_type))
			->where($db->qn('media_type') . '=' . $db->q('file'));
		if (count($in)) {
			$query
				->where('id not in (' . implode(', ', $in) . ')')
				->where($db->qn('id') . ' NOT IN (' . implode(', ', $db->q($in)) . ')');
	...
	public static function getBgColor($file, $folder, $loc_ext_name_com = 'com_ksenmart', $css = true) {
		$path = JPATH_ROOT . DS . 'media' . DS . $loc_ext_name_com . DS . 'images' . DS . $folder . DS . 'original' . DS . basename($file);
		if (!JFile::exists($path)) return '#fff';
	    ...
	    if ($css) {
	    	return 'rgba(' . $colors['red'] . ',' . $colors['green'] . ',' . $colors['blue'] . ',' . (127 - $colors['alpha']) . ')';
	    } else {
	    	return $colors;
	    }
	}
	...
	public static function deleteItemMedia($id = null, $owner_type = null) {
		...
		foreach ($medias as $media) {
			...
			if ($media->media_type == 'file') {
				KSMedia::deleteFile($media->filename, $media->folder);
			}
			if ($media->media_type == 'video') {
			}
		}
