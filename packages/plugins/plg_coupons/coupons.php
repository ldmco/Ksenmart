...
class plgKMDiscountCoupons extends KMDiscountPlugin
{
	...
	function onAfterDisplayKSMCartDefault_content($view, &$tpl = null, &$html)
	{
		...
		if (!empty($coupon_id))
		{
			...
			$coupon_html .= '<form id="km-coupon-form" method="post">';
			$coupon_html .= '	<input type="hidden" name="task" value="unset_discount_coupon">';
			$coupon_html .= '</form>';
			$coupon_html .= KSSystem::loadPluginTemplate($this->_name, $this->_type, $view, 'unset');

			$script = '
			jQuery(document).ready(function(){
			
				jQuery(".ksm-cart").on("click",".km-coupons .btn",function(){
		...
		{
			$coupon_html .= '<form id="km-coupon-form" method="post">';
			$coupon_html .= '	<input type="hidden" name="discount_code" value="" />';
			$coupon_html .= '	<input type="hidden" name="task" value="set_discount_coupon">';
			$coupon_html .= '</form>';
			$coupon_html .= KSSystem::loadPluginTemplate($this->_name, $this->_type, $view, 'set');
	...
	{
		if (empty($view->order->discounts)) return;
		$view->order->discounts = json_decode($view->order->discounts, true);
		if (!is_array($view->order->discounts)) return;
	...
	function onGetDiscountContent($discount_id = null)
	{
		...
		$db->setQuery($query);
		$coupon_id = $db->loadResult();
		if (!empty($created) && empty($coupon_id)) return true;
