...
class plgKMExportimportExport_ym_extend extends KMPlugin {
	...
	function ApiYM(){
		...
		if(!empty($vars)){
			...
			if (!is_array($discounts)) $discounts = array();		
		...
		foreach($rows as $row) {
			...
			foreach($row->regions as $country) {
				if(in_array($this->_region_id, $country)) {
					...
					if(empty($row->price) && $row->price !== 0) $row->price = 0;
					$shippings[] = $row;
				}
			}
		}
	...
	function cartInfo($cart = null){
		if(!isset($cart->delivery) && isset($cart->delivery->region)) return false;
		...
		if(!empty($region)){
			switch($region->type){
				...
				case 'SUBJECT_FEDERATION':
					$name = $region->name;
					if($region->name == 'Москва и Московская область') $name = 'Московская область';
					$query = $this->db->getQuery(true);
					$query->select('id')->from('#__ksenmart_regions')->where('title=' . $this->db->q($name));
					$this->db->setQuery($query);
					$region = $this->db->loadResult();
					if(!empty($region)) $this->_region_id = $region;
				break;
			}
	...
	function onBeforeViewKSMCatalog($view){
		...
		foreach($rows as $row){
			...
			$row->link = $domain . JRoute::_('index.php?option=com_ksenmart&view=product&id=' . $row->id . ':' . $row->alias . '&Itemid=' . KSSystem::getShopItemid(), false, false);
			if(!empty($utm_source)) $row->link .= '?utm_source=' . $utm_source;
    ...
    function getFormData() {
        ...
        if(isset($settings['discounts'])) $data->discounts = json_decode($settings['discounts']->value, true);
		if(empty($settings['postpaid_regions']->value)) $settings['postpaid_regions']->value = '{}';
        ...
        if(isset($settings['utm_source'])) $data->utm_source = $settings['utm_source']->value;
	...
	function onCheckDiscountManufacturers($discount_id = null, $manufacturer_id = null) {
        ...
        if(empty($manufacturer_id)) return true;
        if(empty($this->_discounts[$discount_id]->manufacturers)) return true;
        $manufacturers = json_decode($this->_discounts[$discount_id]->manufacturers, true); 
        if(!count($manufacturers)) return true;
        if(!in_array($manufacturer_id, $manufacturers)) return false;
        return true;
    }
    ...
    function onCheckDiscountCategories($discount_id = null, $category_id = null) {
        ...
        if(empty($this->_discounts[$discount_id]->categories)) return true;
        $categories = json_decode($this->_discounts[$discount_id]->categories, true); 
        if(!count($categories)) return true;
        if(!in_array($category_id, $categories)) return false;
        return true;
    }
