...
class KsenmartRouter extends JComponentRouterBase {
	...
	function build(&$query) {
		...
		switch ($query['view']) {
			...
			case 'product':
				...
				foreach ($config as $key => $val) {
					...
					switch ($key) {
						...
						case 'seo-parent-product':
							...
							break;
						case 'seo-product':
							$slug  = explode(':', $query['id']);
							$alias = '';
							if (!empty($slug[1])) $alias = trim($slug[1]);
							if (empty($alias)) $alias = $this->getAlias('products', $query['id']);
				...
				break;
			case 'catalog':
				if (isset($query['layout']) && $query['layout'] == 'catalog') unset($query['layout']);
				...
				if (isset($query['properties'])) $query['properties'] = \Joomla\Utilities\ArrayHelper::toInteger($query['properties']);
				$search     = false;
				$search_add = 0;
						...
						if (count($query['categories']) == 1) {
							...
							foreach ($config as $key => $val) {
								...
								if ($key == 'seo-parent-category') {
									...
									{
										if (!empty($slug[1])) $alias = trim($slug[1]);
										if (empty($alias)) $alias = $this->getAlias('categories', $category_id);
										if (!empty($alias)) $segments[] = $alias;
									}
								}
							}
				...
				if (isset($query['properties'])) {
					...
					if (!empty($query['properties']) && is_array($query['properties'])) {
						if (!empty($this->_aliases['values'][implode(',', $query['properties'])])) {
							...
							$sql = $db->getQuery(true);
							$sql->select('id,alias,property_id')->from('#__ksenmart_property_values')->where('id IN (' . implode(',', $query['properties']) . ')');
							$db->setQuery($sql);
						...
						foreach ($query['properties'] as $property) {
							if (!isset($property_values[$property])) continue;
							if (!isset($properties[$property_values[$property]->property_id])) $properties[$property_values[$property]->property_id] = array();
							if (!empty($property_values[$property]->alias)) $properties[$property_values[$property]->property_id][] = $property_values[$property]->alias;
							else $properties[$property_values[$property]->property_id][] = 'propertyvalue-' . $property;
						}
						...
						foreach ($properties as $key => $property_values) {
							if (!empty($this->_aliases['properties'][$key])) {
								...
								$sql = $db->getQuery(true);
								$sql->select($db->qn(array('alias', 'range')))->from('#__ksenmart_properties')->where('id=' . $key);
								$db->setQuery($sql);
	...
	function parse(&$segments) {
		...
		switch ($segment) {
			...
			case 'plugin':
				$vars['view'] = 'catalog';
				if (isset($segments[1])) $vars['plugin'] = $segments[1];
				if (count($segments) > 2) {
					$c_segments         = 0;
		...
		if (count($properties) > 0 || count($range_properties) > 0) $vars['properties'] = $properties;
		if (count($range_properties) > 0) $vars['range_properties'] = $range_properties;
		if (count($manufacturers) > 0) $vars['manufacturers'] = $manufacturers;
		if (count($countries) > 0) $vars['countries'] = $countries;

		$check_vars = $vars;
		if (isset($check_properties)) $check_vars['properties'] = array_merge($check_vars['properties'], $check_properties);
		if (isset($category_slug)) $check_vars['categories'][0] = $category_slug;
	...
	private function getAlias($type, $id) {
		if (!empty($this->_aliases[$type][$id])) {
			return $this->_aliases[$type][$id];
		} else {
			$db  = JFactory::getDbo();
			$sql = $db->getQuery(true);
			$sql->select('alias')->from('#__ksenmart_' . $type)->where('id=' . $db->q($id));
			$db->setQuery($sql);
			$alias = $db->loadResult();
			if (!empty($alias)) {
				$this->_aliases['categories'][$id] = $alias;
