...
jQuery(document).ready(function () {
    ...
    jQuery('.ksm-cart').on('click', '#ksm-cart-show-order', function () {
        jQuery('.ksm-cart .default_content').hide();
        jQuery('.ksm-cart-order').show();

        var destination = jQuery('#sp-main-body').offset().top - 40;
        jQuery('body').animate({
    ...
    jQuery('.ksm-cart').on('click', '.ksm-cart-order-next-step', function (e) {
        ...
        jQuery('.ksm-cart-order-form').find('input,textarea,select').filter('[required]').each(function (e) {
            ...
            if (result) result = validator.element('#' + id);
        });
        ...
        var fields = jQuery('.ksm-cart-order-form').serializeArray();
