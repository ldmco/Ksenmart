...
else
{
	foreach($tmpl->getGroup('') as $field) {
		$table_head .= '<th class="' . strtolower($field->type) . ' ' . strtolower($field->fieldname) . '">' . strip_tags($field->label) . '</th>';
	}
...
<div class="row-fluid">
	<div class="subform-repeatable-wrapper subform-table-layout">
		...
		<table class="adminlist table table-striped ksm-repeat-table">
			<thead>
				<tr>
					<?php echo $table_head; ?>
					...
					<?php if (!empty($buttons['add'])):?>
						<div class="btn-group">
							<a class="group-add btn btn-mini button btn-success"> </a>
						</div>
					<?php endif;?>
					</th>
					<?php endif; ?>
