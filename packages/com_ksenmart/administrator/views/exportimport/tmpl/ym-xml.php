...
if (mysql_num_rows($res) > 0) $Itemid = '&amp;Itemid=' . mysql_result($res, 0, 'id');

$query = "select * from {$config->dbprefix}ksenmart_yandeximport where setting='shopname'";
if ($res = mysql_query($query)) $shopname = mysql_result($res, 0, 'value');

$query = "select * from {$config->dbprefix}ksenmart_yandeximport where setting='company'";
if ($res = mysql_query($query)) $company = mysql_result($res, 0, 'value');
...
if ($res = mysql_query($query)) {
    while ($row = mysql_fetch_array($res)) $currencies.= '<currency id="' . $row['code'] . '" rate="' . round($rur_rate / $row['rate'], 4) . '"/>';
}
...
$res = mysql_query($query);
while ($row = mysql_fetch_array($res)) $categories.= '<category id="' . $row['id'] . '" ' . ($row['parent_id'] != 0 ? 'parentId="' . $row['parent_id'] . '"' : '') . '>' . $row['title'] . '</category>';
