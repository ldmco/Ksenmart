...
class KsenMartModelCatalog extends JModelKSAdmin
{
	...
	function getListItems()
	{
		...
		if ($this->params->get('show_products_from_subcategories', 1) == 1)
		{
			$cats       = $categories;
			$categories = array();
			foreach ($cats as $cat)
			{
				$c          = KSMCatalog::getChildCats($cat);
	...
	{
		$this->onExecuteBefore('getCurrentCategories');

		$categories = $this->getState('categories');

		$this->onExecuteAfter('getCurrentCategories', array(&$categories));
	...
	function saveSet($data)
	{
		...
		\Joomla\Utilities\ArrayHelper::toInteger($data['categories']);
		$default_category = 0;
	...
	function getProduct($categories = array())
	{
		...
		$uri                 = $router->build('index.php?option=com_ksenmart&view=product&id=' . $product->id . ':' . $product->alias . '&Itemid=' . KSSystem::getShopItemid());
		$product->link       = str_replace(JUri::base(true) . '/', JUri::root(), $uri->getPath());

		if (count($categories) && $id == 0)
		{
			$is_default = 1;
			foreach ($categories as $category_id)
			{
	...
	function saveProduct($data)
	{
		...
		$data['hot']            = isset($data['hot']) ? $data['hot'] : 0;
		$data['recommendation'] = isset($data['recommendation']) ? $data['recommendation'] : 0;
		$data['is_parent']      = !empty($data['childs']) || $data['add_child'] ? 1 : 0;
		$table                  = $this->getTable('products');
		...
		foreach ($data['categories'] as $category_id)
		{
			...
			$this->setMinPriceCategory($category_id);
			$in[] = $table->id;
		}
		$query = $this->_db->getQuery(true);
		$query->delete('#__ksenmart_products_categories')->where('product_id=' . $id);
			...
			switch ($type)
			{
				case 'text':
					...
					if (!empty($property['text']))
					{
						...
						if (empty($value_row))
						{
							$p_alias = KSFunctions::GenAlias($property['text']);
							$query   = $this->_db->getQuery(true);
				...
				case 'select':
					$count = 0;
					if (!empty($property['generated']))
					{
						$this->generateChilds($data);
						continue;
					}
					foreach ($property as $tmpkey => $tmpvalue)
					{
						if (!is_numeric($tmpkey)) continue;
		...
		{
			$query = $this->_db->getQuery(true);
			$query->update('#__ksenmart_products')->set('select_properties=1')->where('id=' . (int) $id);
			$this->_db->setQuery($query);
			$this->_db->execute();
		}
		...
		$jinput       = JFactory::getApplication()->input;
		$child_groups = $jinput->get('child_groups', array(), 'ARRAY');
		...
		$on_close = '
            if(typeof window.parent.ProductsList != "undefined") {                
                window.parent.ProductsList.refreshList();
            } else {
	...
	function getChild($categories = array())
	{
		...
		$product = KSSystem::loadDbItem($id, 'products');
		$product = KSMedia::setItemMedia($product, 'product');
		if ($id == 0) $product->parent_id = $jinput->getInt('parent_id');
		$product->categories = array();
		$product->properties = array();
		$product->childs     = array();
	...
	private function generateChilds($data = array())
	{
		$db = JFactory::getDbo();
		foreach ($data['properties'] as $property_id => $property)
		{
			if (empty($property['generated'])) continue;
			$query = $db->getQuery(true);
			$query->select('title')->from('#__ksenmart_properties')->where('id=' . (int) $property_id);
			$db->setQuery($query);
			$property_title = $db->loadResult();
			foreach ($property as $tmpkey => $tmpvalue)
			{
				if (!is_numeric($tmpkey)) continue;
				$query = $db->getQuery(true);
				$query->select('title')->from('#__ksenmart_property_values')->where('id=' . (int) $tmpkey);
				$db->setQuery($query);
				$value_title = $db->loadResult();
				if (!empty($value_title))
				{
					$childData                                      = array();
					$childData['title']                             = $data['title'] . ' ' . $property_title . ' ' . $value_title;
					$childData['properties']                        = array();
					$childData['properties'][$property_id][$tmpkey] = $tmpvalue;
					$childData['parent_id']                         = $data['id'];
					$childData['type']                              = 'child';
					$childData['categories']                        = $data['categories'];
					$childData['price']                             = $data['price'];
					$childData['in_stock']                          = $data['in_stock'];
					$childData['price_type']                        = 1;

					$this->saveChild($childData);
				}
			}
		}
	}
	...
	function saveChild($data)
	{
		...
		$jinput    = JFactory::getApplication()->input;
		$parent_id = $jinput->getInt('parent_id');
	...
	public function saveCategory($data)
	{
		...
		$this->setMinPriceCategory($id);

		$on_close = '
			if(typeof window.parent.CategoriesModule != "undefined") window.parent.CategoriesModule.refresh();
			if(typeof window.parent.updateCategories != "undefined") window.parent.updateCategories();
		';
		$return   = array('id' => $id, 'on_close' => $on_close);
	...
	function saveManufacturer($data)
	{
		...
		$on_close = '
			if(typeof window.parent.ManufacturersModule != "undefined") window.parent.ManufacturersModule.refresh();
			if(typeof window.parent.updateManufacturers != "undefined") window.parent.updateManufacturers();
		';
		$return   = array('id' => $id, 'on_close' => $on_close);
	...
	public function setMinPriceCategory($cid = 0)
	{
		if (empty($cid)) return false;
		if (!$this->params->get('show_minprice_categories')) return false;
		$where = array(
			'p.published=1'
		);

		if ($this->params->get('show_out_stock') != 1) $where[] = "(p.in_stock>0)";
		$query = $this->_db->getQuery(true);
		$query->select('min(p.price/c.rate)')->from('#__ksenmart_products as p');
		$query->join("INNER", "#__ksenmart_products_categories as pc on p.id=pc.product_id");
		$where[] = "pc.category_id=" . (int) $cid;
		$query->join("LEFT", "#__ksenmart_currencies as c on p.price_type=c.id");
		$query->where($where);
		$this->_db->setQuery($query);
		$minprice = $this->_db->loadResult();

		$category           = new stdClass();
		$category->id       = $cid;
		$category->minprice = $minprice;
		$result             = $this->_db->updateObject('#__ksenmart_categories', $category, 'id');
	}
