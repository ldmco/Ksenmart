...
class KsenMartModelOrders extends JModelKSAdmin
{
	...
	{
		$this->onExecuteBefore('getOrder', array(&$vars));
		...
		if (isset($vars['discounts']))
		{
			$discount_ids     = $vars['discounts'];
			...
			foreach ($discount_ids as $discount_id)
			{
				...
				{
					$new_discounts[$discount_id] = $order->discounts[$discount_id];
				}
				else
				{
					$query = $this->_db->getQuery(true);
					...
					$this->_db->setQuery($query);
					$discount = $this->_db->loadObject();
					if (!empty($discount))
						$new_discounts[$discount_id] = json_decode($discount->params);
				}
			}
			$order->discounts = json_encode($new_discounts);
		}
		...
		foreach ($order->items as &$item)
		{
			...
			if (count($product))
			{
				...
				if (!empty($product->purchase_price))
					$order->purchase_sum += $product->purchase_price;
				else
					$order->purchase_sum += $product->price;
		...
		$order->costs = array(
			...
			'purchase_cost'     => $order->purchase_sum,
			'purchase_cost_val' => KSMPrice::showPriceWithTransform($order->purchase_sum),
	...
	function updateOrderDiscounts($order_id = null, $discount_ids = array())
	{
		...
		{
			$query = $this->_db->getQuery(true);
			$query->select('discounts')->from('#__ksenmart_orders')->where('id=' . (int) $order_id);
			$this->_db->setQuery($query);
			$old_discounts = $this->_db->loadResult();
		}
		else
		{
			$old_discounts = '[]';
		...
		$old_discounts = json_decode($old_discounts, true);
		$new_discounts = array();
		foreach ($discount_ids as $discount_id => $discount)
		{
			if (isset($old_discounts[$discount_id]))
			{
				$new_discounts[$discount_id] = $old_discounts[$discount_id];
			}
			elseif ($discount_id < 0)
			{
				$discount_param        = new stdClass();
				$discount_param->value = $discount['cost'];
				$discount_param->type  = 1;
				$new_discounts['-1']   = $discount_param;
			}
			else
			{
				$query = $this->_db->getQuery(true);
				$query->select('params')->from('#__ksenmart_discounts')->where('id=' . (int) $discount_id);
				$this->_db->setQuery($query);
				$discount_param = $this->_db->loadResult();
				if (!empty($discount_param))
					$new_discounts[$discount_id] = json_decode($discount_param);
			}
		}

		return json_encode($new_discounts);
	}
	...
	function saveOrderStatus($data)
	{
		...
		$data['withdraw'] = isset($data['withdraw']) ? $data['withdraw'] : 0;
