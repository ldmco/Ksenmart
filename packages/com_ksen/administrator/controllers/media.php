...
class KsenControllerMedia extends KsenController {
	...
	function upload_image() {
		...
		$file   = $jinput->files->get('files', null, 'raw');
		if (empty($file)) {
			$file = JRequest::getVar('Filedata', '', 'files', 'array');
		}
		$extension = $jinput->get('extension', null);
		...
		$properties  = JImage::getImageFileProperties($file['tmp_name']);
		$imageObject = $imageObject->resize($properties->width, $properties->height, false);
		...
		try {
			$imageObject->toFile($fileName, $type, array(
				'quality',
				100
			));
	...
	function upload_file() {
		...
		$pathinfo = pathinfo($file['name']);
		if ($rename) {
			$fileName = microtime(true) . '.' . $pathinfo['extension'];
		} else {
			$fileName = $file['name'];
		}
		copy($file['tmp_name'], $path . DS . $fileName);
