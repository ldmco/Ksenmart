...
class modKMShippingHelper 
{
    ...
    public function getShippings($region_id) 
	{
		$db = JFactory::getDBO();
        $shippings = array();
    ...
    public function getPayments($region_id) 
	{
		$db = JFactory::getDBO();
        $payments = array();
		...
		foreach($db_payments as $db_payment) 
		{
			...
			$db_payment->icon = !empty($db_payment->filename) ? KSMedia::resizeImage($db_payment->filename, $db_payment->folder, 20, 20, json_decode($db_payment->params_f, true)) : '';			
			$payments[] = $db_payment;
		}			
	...
	public function checkRegion($regions, $region_id)
	{
