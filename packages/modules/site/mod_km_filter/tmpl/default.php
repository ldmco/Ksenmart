...
<div class="ksm-module-filter ksm-block <?php echo $class_sfx?>">
    <form action="<?php echo $form_action; ?>" method="get" data-price_min="<?php echo (int) $price_min; ?>" data-price_max="<?php echo (int) $price_max; ?>" data-price_less="<?php echo (int) $price_less; ?>" data-price_more="<?php echo (int) $price_more; ?>" data-show_filter_button="<?php echo $params->get('show_filter_button', 0); ?>">
        <div class="ksm-module-filter-selecteds" style="display:<?php echo (!empty($selected) ? 'block' : 'none'); ?>">
		    <?php if (!empty($selected['manufacturers'])): ?>
                <div class="ksm-module-filter-selected-block">
				    ...
				    <?php foreach ($manufacturers as $manufacturer): ?>
					    <?php if (!$manufacturer->selected) continue; ?>
		    ...
		    <?php if (!empty($selected['properties'])): ?>
			    <?php foreach ($properties as $property): ?>
				    <?php if ($property->view == 'none') continue; ?>
				    <?php if ($property->view == 'none') continue; ?>
				    <?php $count = 0; ?>
				    <?php foreach($property->values as $value){ ?>
					    <?php if (!$value->selected) continue; ?>
					    <?php if(!$count): ?>
                            <div class="ksm-module-filter-selected-block">
			    ...
			    <?php endforeach; ?>
		    <?php endif; ?>
            <a href="#" class="ksm-module-filter-button-clear"><?php echo JText::_('MOD_KM_FILTER_SELECTED_CLEAR')?></a>
        </div>
					...
					<li class="ksm-module-filter-block-listing-item <?php echo $manufacturer->selected?' active':''; ?>" data-id="<?php echo $manufacturer->id; ?>">
						<label>
							<?php if ($mod_params['manufacturer']['view'] == 'images' && $manufacturer->image !='' ){ ?>					
								<input style="display:none;" onchange="KMChangeFilter(this);" type="checkbox" name="manufacturers[]" value="<?php echo $manufacturer->id; ?>" <?php if ($manufacturer->selected) echo 'checked'; ?> />
								<img src="<?php echo $manufacturer->image; ?>" alt="<?php echo $manufacturer->title; ?>" />
							<?php }elseif ($mod_params['manufacturer']['view'] == 'checkbox'){ ?>
								<input onchange="KMChangeFilter(this);" type="checkbox" name="manufacturers[]" value="<?php echo $manufacturer->id; ?>" <?php if ($manufacturer->selected) echo 'checked'; ?> />
								<span><?php echo $manufacturer->title; ?></span>
							<?php }elseif ($mod_params['manufacturer']['view'] == 'radio'){ ?>
								<input onchange="KMChangeFilter(this);" type="radio" name="manufacturers[]" value="<?php echo $manufacturer->id; ?>" <?php if ($manufacturer->selected) echo 'checked'; ?> />
								<span><?php echo $manufacturer->title; ?></span>
							<?php }else{ ?>
								<input style="display:none;" onchange="KMChangeFilter(this);" type="checkbox" name="manufacturers[]" value="<?php echo $manufacturer->id; ?>" <?php if ($manufacturer->selected) echo 'checked'; ?> />
								<span><?php echo $manufacturer->title; ?></span>
		...
		<div class="ksm-module-filter-block ksm-module-filter-block-property ksm-module-filter-block-property-<?php echo $property->property_id?> ksm-module-filter-block-<?php echo $property->display?> ksm-module-filter-block-<?php echo $property->view?>"
             data-property_id="<?php echo $property->property_id ?>">
			<ul class="ksm-module-filter-block-listing">
					...
					<li class="ksm-module-filter-block-listing-item <?php echo $value->selected?' active':''; ?>" data-id="<?php echo $value->id; ?>">
						<label>
							...
							<?php }elseif ($property->view == 'checkbox'){ ?>
								<input onchange="KMChangeFilter(this);" type="checkbox" name="properties[]" value="<?php echo $value->id; ?>" <?php if ($value->selected) echo 'checked'; ?> />
								<span><?php echo $value->title; ?></span>
							<?php }elseif ($property->view == 'radio'){ ?>
								<input onchange="KMChangeFilter(this);" type="radio" name="properties[<?php echo $property->property_id; ?>]" value="<?php echo $value->id; ?>" <?php if ($value->selected) echo 'checked'; ?> />
								<span><?php echo $value->title; ?></span>
							<?php }else{ ?>
								<input style="display:none;" onchange="KMChangeFilter(this);" type="checkbox" name="properties[]" value="<?php echo $value->id; ?>" <?php if ($value->selected) echo 'checked'; ?> />
								<span><?php echo $value->title; ?></span>
					...
					<li class="ksm-module-filter-block-listing-item <?php echo $country->selected?' active':''; ?>" data-id="<?php echo $country->id; ?>">
						<label>
							<?php if ($mod_params['country']['view'] == 'images' && $country->image!=''){ ?>					
								<input style="display:none;" onchange="KMChangeFilter(this);" type="checkbox" name="countries[]" value="<?php echo $country->id; ?>" <?php if ($country->selected) echo 'checked'; ?> />
								<img src="<?php echo $country->image; ?>" alt="<?php echo $country->title; ?>" />
							<?php }elseif ($mod_params['country']['view'] == 'checkbox'){ ?>
								<input onchange="KMChangeFilter(this);" type="checkbox" name="countries[]" value="<?php echo $country->id; ?>" <?php if ($country->selected) echo 'checked'; ?> />
								<span><?php echo $country->title; ?></span>
							<?php }elseif ($mod_params['country']['view'] == 'radio'){ ?>
								<input onchange="KMChangeFilter(this);" type="radio" name="countries[]" value="<?php echo $country->id; ?>" <?php if ($country->selected) echo 'checked'; ?> />
								<span><?php echo $country->title; ?></span>
							<?php }else{ ?>
								<input style="display:none;" onchange="KMChangeFilter(this);" type="checkbox" name="countries[]" value="<?php echo $country->id; ?>" <?php if ($country->selected) echo 'checked'; ?> />
								<span><?php echo $country->title; ?></span>
